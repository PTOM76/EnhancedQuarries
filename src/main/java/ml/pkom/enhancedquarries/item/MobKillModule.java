...
public class MobKillModule extends Item {
    public MobKillModule(Settings settings) {
        super(settings);
    }
        ...
        if (world.getBlockState(blockPos).getBlock() instanceof Quarry) {
            if (world.getBlockEntity(blockPos) != null && world.getBlockEntity(blockPos) instanceof QuarryTile) {
                ...
                if (quarry.isSetMobKill()) {
                    context.getPlayer().sendMessage(TextUtil.translatable("message.enhanced_quarries.mob_kill_module.1"), false);
                    return ActionResult.PASS;
                }
                if (quarry.isSetMobDelete()) {
                    context.getPlayer().sendMessage(TextUtil.translatable("message.enhanced_quarries.mob_kill_module.2"), false);
                    return ActionResult.PASS;
                }
