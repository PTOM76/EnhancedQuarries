        ...
        if (world.getBlockState(blockPos).getBlock() instanceof Quarry) {
            if (world.getBlockEntity(blockPos) != null && world.getBlockEntity(blockPos) instanceof QuarryTile) {
                ...
                if (quarry.isSetMobDelete()) {
                    context.getPlayer().sendMessage(new TranslatableText("message.enhanced_quarries.mob_delete_module.1"), false);
                    return ActionResult.PASS;
                }
                if (quarry.isSetMobKill()) {
                    context.getPlayer().sendMessage(new TranslatableText("message.enhanced_quarries.mob_delete_module.2"), false);
                    return ActionResult.PASS;
                }
