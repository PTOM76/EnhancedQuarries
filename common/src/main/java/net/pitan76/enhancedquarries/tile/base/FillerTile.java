    ...
    public ItemStack getInventoryStack() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (EQStorageBoxUtil.isStorageBox(stack)) {
                ItemStack stackInBox = StorageBoxHooks.getStackInStorageBox(stack);
    ...
    public boolean tryPlacing(BlockPos blockPos, Block block, ItemStack stack) {
        ...
        if (world.setBlockState(blockPos, BlockStateUtil.getMidohraDefaultState(block))) {
            ...
            if (EQStorageBoxUtil.isStorageBox(latestGotStack)) {
                if (StorageBoxHooks.hasStackInStorageBox(latestGotStack)) {
                    int countInBox = StorageBoxHooks.getAmountInStorageBox(latestGotStack);
                    countInBox--;
                    if (countInBox < 1) {
                        StorageBoxHooks.setStackInStorageBox(latestGotStack, ItemStackUtil.empty());
                    } else {
                        StorageBoxHooks.setAmountInStorageBox(latestGotStack, countInBox);
                    }
